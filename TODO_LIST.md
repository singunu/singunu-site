...
- [x] 🤖 답변 마크다운(`**` 등) 제거 — 시스템 프롬프트 규칙(PersonaService §5 형식) + 프론트 스트립(`ChatApp.tsx` `stripMarkdown`, 누적 텍스트 렌더 직전 적용)
- [x] 🤖 `public_persona.md` 시점 분리 — `[현재 상태 — 기준일: 2026-07-07]` 섹션 신설, 상대 표현 제거, "기준일 이후는 폴백 유도" 지침(프롬프트 §6 + 문서 §7)
- [x] 🙋 먹거리·카페 등 취향 원본 작성 — `me/_인생_질문지.md`(20문항 카테고리) 기입 완료 → `me/20_일상/취향과_라이프스타일.md`로 정리
...
- [x] 🤖 취향(영화·책·애니) 대폭 확장 + 프레이밍 수정 배포(2026-07-08, 커밋 2ff2f56) — 볼트 `취향_영화.md`(왓챠 988편)·`독서_리스트`(90권)에서 영화 40여편·책 18권·애니 9편 확충, 대화지침 §8("유명작 2~3개로 좁히지 말 것, 인셉션·셔터아일랜드 대표작으로 앞세우지 말 것"). 라이브 검증: "딱 3개 골라봐"→시네도키뉴욕·기생충·헤어질결심(인셉션/셔터 아님)
