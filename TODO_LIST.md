...
- [ ] 🙋 Anthropic API 키 발급 → https://console.anthropic.com (Billing에서 월 지출 한도 설정 권장: $5)

## Phase 1 — 페르소나 데이터 파이프라인
- [x] 🤖 `me/` 볼트 → `persona/public_persona.md` 공개용 큐레이션 초안 생성
- [x] 🤖 민감정보 제외 체크리스트 작성 (`persona/REVIEW_CHECKLIST.md`)
- [ ] 🙋 **공개 범위 최종 검수** — `persona/public_persona.md`를 읽고 빼고 싶은 내용 삭제/수정 (반드시 사람이!)
- [ ] 🤖 검수 반영 후 확정본 동결
...
- [ ] 🙋 Gmail 앱 비밀번호 발급 (2단계 인증 → 앱 비밀번호) → 전달
